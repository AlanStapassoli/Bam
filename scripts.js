...
const Storage = {
  get() {
    return JSON.parse(localStorage.getItem('Bam:transaction')) || [];
  },
  set(transaction) {
    localStorage.setItem('Bam:transaction', JSON.stringify(transaction));
  }
}
...
const DOM = {
  ...
  innerHTMLTransaction(transaction, index) {
    ...
    const html = `
      ...
      <td class="descriptionCli">${transaction.descriptionCli}</td>
      <td>
...
const Utils = {
  ...
  createPDF(e) {
    ...
    doc.text('', 105, 285, null, null, "center");
...
const Form = {
  getValuesAdd() {
    return {
      ...
      descriptionCli: document.querySelector('.add input#descriptionCli').value,
    }
  },
  getValuesEdit() {
    return {
      ...
      descriptionCli: document.querySelector('.edit input#descriptionCli').value,
    }
  },
  verifyErrors(field) {
  ...
  formatValuesAdd() {
    let { description, amount, date, descriptionCli } = Form.getValuesAdd();
  ...
  formatValuesEdit() {
    let { transactionId, description, amount, date, descriptionCli } = Form.getValuesEdit();
    amount = Utils.formatAmount(amount);
    date = Utils.formatDate(date);
  ...
  submit(event, action) {
    ...
    if(action === 'edit'){
      const {transactionId, description, amount, date, descriptionCli} = Form.formatValuesEdit();
      Form.editTransaction(transactionId, {description, amount, date, descriptionCli});
      Form.clearFields();
      Modal.toggle('edit')
    }
  }
